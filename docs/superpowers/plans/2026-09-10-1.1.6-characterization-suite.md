...
**Architecture:** Isolate test execution into a standalone `pytest.ini` with `pythonpath = .` and exact pinned dependencies in `requirements-test.txt`. Organize characterization into layered unit and integration tests under `tests/` using hermetic filesystem and boundary mocking, capturing observable contracts in a documented matrix in `docs/baseline-1.1.6.md`.

**Tech Stack:** Python 3.14 / 3.7+ compatibility, `pytest==9.1.1`, Click `CliRunner`, `unittest.mock`.
...
- Exact `==` pins for test infrastructure; do not use open-ended `>=` constraints in baseline requirements.
...
- Do not classify suspected behavior as a defect until reproduced by code and/or a reproducible test.
...
Remove the semantic patterns `test_*` and `*test*`. Retain `test/` and all other rules.
...
### Task 3: Characterize `djstartlib.version` (`tests/unit/test_version.py`)
...
- Produces: Characterization of `BC-VER-01`, `BC-VER-02`, `BC-VER-03`, `BC-VER-04`, `BC-VER-05`.

- [ ] **Step 1: Write failing/characterizing tests in `tests/unit/test_version.py`**
...
def test_check_available_arithmetic_quirk(capsys):
    # Demonstrates known quirk where sum([1, 10, 0]) = 11, but sum([2, 0, 0]) = 2
    # Even if remote is 2.0.0, if current were 1.10.0, sum would fail semver.
    with patch("djstartlib.version.latest_version", return_value=("1.0.9", [1, 0, 9])):
        with patch("djstartlib.version.current_version", return_value=[1, 1, 6]):
            # 1+0+9=10 > 1+1+6=8 -> reports new update available even though 1.0.9 is older!
            result = check_available()
            captured = capsys.readouterr()
            assert result is True
...
- [ ] **Step 2: Run pytest to verify all version tests pass**
...
git commit -m "test: characterize djstartlib.version contracts and quirks"
```

---

### Task 4: Characterize `djstartlib.models.utils.Environment` (`tests/unit/test_environment.py`)
...
- Produces: Characterization of `BC-ENV-01`, `BC-ENV-02`.

- [ ] **Step 1: Write tests in `tests/unit/test_environment.py`**
...
- [ ] **Step 2: Run pytest to verify all environment tests pass**
...
### Task 5: Characterize `ProjectManager` (`tests/unit/test_project_manager.py`)
...
- Produces: Characterization of `BC-PRJ-01` through `BC-PRJ-08`.

- [ ] **Step 1: Write tests in `tests/unit/test_project_manager.py`**
...
- [ ] **Step 2: Run pytest to verify all project manager tests pass**
...
### Task 6: Characterize `AppManager` (`tests/unit/test_app_manager.py`)
...
- Produces: Characterization of `BC-APP-01` through `BC-APP-07`.

- [ ] **Step 1: Write tests in `tests/unit/test_app_manager.py`**
...
- [ ] **Step 2: Run pytest to verify all app manager tests pass**
...
### Task 7: Characterize Helper Templates & Command Runners (`tests/unit/test_helper.py`)
...
- Produces: Characterization of `BC-HLP-01`, `BC-HLP-02`.

- [ ] **Step 1: Write tests in `tests/unit/test_helper.py`**
...
- [ ] **Step 2: Run pytest to verify all helper tests pass**
...
git commit -m "test: characterize helper template and execution wrappers"
```

---

### Task 8: Characterize `djstartlib.main` CLI (`tests/unit/test_cli.py`)
...
- Produces: Characterization of `BC-CLI-01` through `BC-CLI-05`.

- [ ] **Step 1: Write tests in `tests/unit/test_cli.py`**
...
def test_cli_default_invocation(isolated_workdir):
    ...
    with patch("djstartlib.main.DjangoStart") as mock_dj:
        ...
        # Verify DjangoStart initialized with absolute path for default 'env'
...
def test_cli_custom_options(isolated_workdir):
    ...
    with patch("djstartlib.main.DjangoStart") as mock_dj:
        ...
        # Verify deprecation warning for -v
        assert "Please Don't Use This Option is Deprecated" in result.output
```

- [ ] **Step 2: Run pytest to verify all CLI tests pass**
...
### Task 9: Characterize End-to-End Orchestration (`tests/integration/test_scaffold_orchestration.py`)
...
- Produces: Characterization of `BC-ORCH-01`.
...
git commit -m "test: characterize end-to-end DjangoStart orchestration lifecycle"
```

---

### Task 10: Baseline Documentation & Contract Matrix (`docs/baseline-1.1.6.md`)
...
1. Environment and historical platform assumptions of 1.1.6.
2. Complete Behavior Contract Matrix with all 29 contracts, classifications (`intended`, `legacy-reliance`, `known-defect`), and corresponding tests.
3. Documented reproduction of quirks:
   - `sum(var_int)` version comparison logic.
...
Run: `.venv/bin/pytest`
Expected: All 39 unit and integration tests PASS.
