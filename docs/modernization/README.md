...
2. [Python & Django Ecosystem Research (September 2026)](ecosystem-research-2026.md)
   ...
   - Official Django support roadmap (Django 5.2 LTS, Django 6.1 released August 5, 2026 / 6.1.1 September 2, 2026, Django 6.2 LTS April 2027, and DEP 20 annual CalVer proposal for 2028.0 with uniform 3-year support)
...
4. [Target Product Architecture](target-architecture.md)
   ...
   - Recipe & Profile engine: orthogonal archetype profiles (`standard`, `api`, `minimal`) combinable with framework release tracks (`--django 6.1` default, `--django 5.2` / `--django lts`)
...
|---|---|---|
| [ADR-001](adr/ADR-001-python-support-policy.md) | Python Support Policy | Support Python 3.12 - 3.14 runtime; isolated Python 3.11 characterization suite; retire Python <= 3.11 at 2.0 GA. |
| [ADR-002](adr/ADR-002-django-support-policy.md) | Django Support Policy | Support Django 6.1 (default, released Aug 2026) and Django 5.2 LTS; strictly pin tested patch releases; track DEP 20 CalVer proposal. |
...
| [ADR-009](adr/ADR-009-recipe-and-profile-architecture.md) | Recipe & Profile Architecture | Modular declarative recipes with orthogonal archetype profiles (`standard`, `api`, `minimal`) and framework release tracks; no executable third-party code. |
