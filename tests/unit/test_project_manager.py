...
def test_project_paths(isolated_workdir):
    """Characterize BC-PRJ-01: Project paths initialization for urls.py and settings.py."""
    pm = ProjectManager(project="mysite", app="blog")
    assert pm.urls_path == os.path.join(str(isolated_workdir), "mysite/urls.py")
    assert pm.settings_path == os.path.join(str(isolated_workdir), "mysite/settings.py")


def test_create_project_when_new(isolated_workdir):
    """Characterize BC-PRJ-02: Project creation when directory does not exist."""
    pm = ProjectManager(project="mysite", app="blog")
    with patch("djstartlib.models.project_manager.executable_django_command") as mock_cmd:
        pm.create_project()
        mock_cmd.assert_called_once_with("startproject mysite .")


def test_create_project_when_exists(isolated_workdir, capsys):
    """Characterize BC-PRJ-03: Project creation warns and skips command when project directory exists."""
...
def test_update_settings_inserts_app_before_closing_bracket(isolated_workdir):
    """Characterize BC-PRJ-04: update_settings inserts app name before closing bracket in settings.py."""
...
def test_update_settings_warns_when_already_installed(isolated_workdir, capsys):
    """Characterize BC-PRJ-05: update_settings warns when app is already present in settings.py."""
    proj_dir = isolated_workdir / "mysite"
    proj_dir.mkdir()
    settings_file = proj_dir / "settings.py"
    settings_file.write_text("INSTALLED_APPS = [\n\t'blog',\n]\n", encoding="utf-8")
...
def test_update_import_statement_inserts_include(isolated_workdir):
    """Characterize BC-PRJ-06: update_import_statment inserts include before path import."""
    pm = ProjectManager(project="mysite", app="blog")
...
def test_update_import_statement_skips_when_combined_present(isolated_workdir):
    """Characterize BC-PRJ-06: update_import_statment returns 0 and skips insertion if path, include already imported."""
    pm = ProjectManager(project="mysite", app="blog")
    pm.line_list = ["from django.urls import path, include\n", "urlpatterns = []\n"]
...
def test_update_urls_wiring(isolated_workdir):
    """Characterize BC-PRJ-07: update_urls inserts include statement and app urlpatterns wiring."""
    proj_dir = isolated_workdir / "mysite"
    proj_dir.mkdir()
...
def test_update_urls_warns_when_already_present(isolated_workdir, capsys):
    """Characterize BC-PRJ-08: update_urls warns when view path is already present in urls.py."""
    proj_dir = isolated_workdir / "mysite"
    proj_dir.mkdir()
    urls_file = proj_dir / "urls.py"
...
def test_helper_delegations():
    """Characterize ProjectManager static helper delegations to module-level helper functions."""
    ...
    with patch("djstartlib.models.project_manager.requirements_extract") as m_req:
